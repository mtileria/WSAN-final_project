  ...
  while (xbee.available()) {     // If anything comes in Serial2 (pins 0 & 1)
          ...
          if (inChar == '%') {
              ...
              if (type == "M")
              {
                   ...
                   if (timed_out){   // i am available to get paired
                       buffer_RX = "A" + rec_ID + String(state) +"B0";
                       TX(buffer_RX, source_addr);
                   }else{ 
                       buffer_RX = "A" + rec_ID +String(state)+ "B1"; //I am busy
                       TX(buffer_RX, source_addr);
                   }
             ...
             // xbee.print("AHi%");
              ...
              if (type == "A")
              {
               // Serial.println("I AM in");
                ...
                if (!timed_out){    //not timeout
                    if(rec_addr.startsWith(TX_dest_addr)){  //got ack from last TX node
                      if (rec_block == "B0"){    //other node is not busy
                       ...
                       }else if(rec_block == "B1"){  //other node is busy
                          //try to pair with another neighbour node
                          Serial.println("try to pair with another neighbour node");
                       }
